...
class FinaglePostgresContext[N <: NamingStrategy](
  ...
  def this(naming: N, config: FinaglePostgresContextConfig) =
    this(naming, config.client)
  def this(naming: N, config: Config) =
    this(naming, FinaglePostgresContextConfig(config))
  ...
  override type NullChecker = FinaglePgNullChecker

  class FinaglePgNullChecker extends BaseNullChecker {
    override def apply(v1: Index, v2: ResultRow): Boolean = {
      v2.getAnyOption(v1).exists(_ != null)
    }
  }

  implicit val nullChecker: NullChecker = new FinaglePgNullChecker()
  ...
  def executeQuery[T](
    ...
    withClient(
      _.prepareAndQuery(sql, prepared: _*)(extractor(_, ())).map(_.toList)
    )
  }
  ...
  def executeActionReturning[T](
    ...
    logger.logQuery(sql, params)
    withClient(
      _.prepareAndQuery(expandAction(sql, returningAction), prepared: _*)(
        extractor(_, ())
      )
    ).map(v => handleSingleResult(sql, v.toList))
  }
  ...
  override private[getquill] def prepareParams(
      statement: String,
      prepare: Prepare
  ): Seq[String] = {
