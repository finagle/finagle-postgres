...
object RowDecoder extends RowDecoder0 {
  ...
  class HConsOption[K <: Symbol, H, T <: HList](implicit
    ...
    @inline final def apply(
        row: Row
    )(implicit columnNamer: ColumnNamer): FieldType[K, Option[H]] :: T = {
      field[K](row.getOption[H](columnNamer(colName.value.name))) :: decodeTail(
        row
      )(columnNamer)
    }
  }
  ...
  implicit def joined[A <: Product, B <: Product, AL <: HList, BL <: HList](
      implicit
    ...
    @inline final def apply(
        row: Row
    )(implicit columnNamer: ColumnNamer): (A, B) = {
      (
...
trait RowDecoder0 {
  ...
  class GenericProduct[T <: Product, L <: HList](implicit
    ...
    @inline final def apply(row: Row)(implicit columnNamer: ColumnNamer): T =
      gen.from(decodeL(row)(columnNamer))
  }
  ...
  class HCons[K <: Symbol, H, T <: HList](implicit
    ...
    @inline final def apply(
        row: Row
    )(implicit columnNamer: ColumnNamer): FieldType[K, H] :: T = {
      field[K](row.get[H](columnNamer(colName.value.name))) :: decodeTail(row)(
        columnNamer
      )
    }
  }
