  ...
  object snake {
    implicit object snake extends ColumnNamer {
      ...
      private val camelRegexes = List(
        ...
        """([A-Z]+)([A-Z])([a-z])""".r -> ((m: Match) =>
          s"${m.group(1)}_${m.group(2)}${m.group(3)}"
        ),
        """([^0-9_])([0-9])""".r -> ((m: Match) =>
          s"${m.group(1)}_${m.group(2)}"
        ),
      ...
      def apply(scalaName: String): String = camelRegexes
        .foldLeft(scalaName) { case (str, (regex, replacer)) =>
          regex.replaceAllIn(str, replacer)
        }
        .toLowerCase()
    }
  }
