...
object Updates {
  ...
  ): Updates = new Updates(toList(mapper(o)).flatten.map { case (k, v) =>
    columnNamer(k) -> v
  })
  ...
  def apply[P <: Product, L <: HList, MP <: HList](p: P)(implicit
      ...
      columnNamer: ColumnNamer
  ): Updates = new Updates(toList(mapper(gen.to(p))).map { case (k, v) =>
    columnNamer(k) -> v
  })
  ...
  object updateWithOptions extends Poly1 {
    implicit def cases[K <: Symbol, T](implicit
        ...
        name: Witness.Aux[K]
    ) = at[FieldType[K, Option[T]]] { opt =>
      opt.map { v =>
        name.value.name -> Param(v)
      }
    }
  }
}
