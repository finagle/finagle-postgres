...
object Generators {
  ...
  implicit val arbBD: Arbitrary[BigDecimal] = Arbitrary(
    for {
      precision <- Gen.choose(1, 32)
      scale <- Gen.choose(-precision, precision)
      digits <- Gen.listOfN[Char](precision, Gen.numChar)
    } yield BigDecimal(
      BigDecimal(digits.mkString).bigDecimal.movePointLeft(scale)
    )
  )
  ...
  implicit val arbDate = Arbitrary[LocalDate](for {
    julian <- Gen.choose(
      1721060,
      5373484
    ) // Postgres date parser doesn't like dates outside year range 0000-9999
  ...
  implicit val arbTimestamp = Arbitrary[LocalDateTime](
    for {
      milli <- Gen.posNum[Long]
  ...
  implicit val arbTimestampTz = Arbitrary[ZonedDateTime](
    for {
      milli <- Gen.posNum[Long]
  ...
  implicit val arbZoneOffset = Arbitrary(
    Gen.choose(-12, 12).map(ZoneOffset.ofHours)
  )

  implicit val arbInterval = Arbitrary(
    for {
  ...
  implicit val arbUTF8String = Arbitrary(arbitrary[String].filter { str =>
    utf8.canEncode(str) && !str.contains('\u0000')
  })
