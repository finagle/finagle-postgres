...
import com.twitter.finagle.postgres.Param.convert
  ...
  for {
    ...
    def getConcurrentClient(maxConcurrency: Int): PostgresClientImpl = {
      val client = Postgres
        ...
        .conditionally(
          useSsl,
          c => sslHost.fold(c.withTransport.tls)(c.withTransport.tls(_))
        )
    ...
    def cleanDb(client: PostgresClient): Unit = {
      val dropQuery = client.executeUpdate(
        "DROP TABLE IF EXISTS %s".format(IntegrationSpec.pgTestTable)
      )
      ...
      Try(
        Await.result(client.execute("create extension hstore"))
      ) // enable hstore type
    }
    ...
    "A postgres client" should {
      "insert and select rows" in {
        ...
        val selectQuery = client.select(
          "SELECT * FROM %s WHERE str_field='hello' ORDER BY timestamp_field"
        ...
        firstRow.getOption[Instant]("timestamp_field") must equal(
          Some(
      ...
      "update a row" in {
        ...
        val updateQuery = client.executeUpdate(
          "UPDATE %s SET str_field='hello_updated' where int_field=4567".format(
        ...
        val selectQuery = client.select(
          "SELECT * FROM %s WHERE str_field='hello_updated'".format(
            IntegrationSpec.pgTestTable
          )
        )(identity)
        ...
        resultRows.head.getOption[String]("str_field") must equal(
          Some("hello_updated")
        )
      }
      ...
      "select rows via a prepared query" in {
        ...
        val preparedQuery = client.prepareAndQuery(
          "SELECT * FROM %s WHERE str_field=$1 AND bool_field=$2".format(
            IntegrationSpec.pgTestTable
          ),
          Param("hello"),
          Param(true)
      ...
      "execute an update via a prepared statement" in {
        ...
        val preparedQuery = client.prepareAndExecute(
          "UPDATE %s SET str_field = $1 where int_field = 4567".format(
            IntegrationSpec.pgTestTable
          ),
          Param("hello_updated")
        )
        ...
        val resultRows = Await.result(
          client.select(
            "SELECT * from %s WHERE str_field = 'hello_updated' AND int_field = 4567"
              .format(IntegrationSpec.pgTestTable)
          )(identity)
        )
      ...
      "execute an update via a prepared statement using a Some(value)" in {
        ...
        val resultRows = Await.result(
          client.select(
            "SELECT * from %s WHERE str_field = 'hello_updated_some' AND int_field = 4567"
              .format(IntegrationSpec.pgTestTable)
          )(identity)
        )
      ...
      "execute an update via a prepared statement using a None" in {
        ...
        val resultRows = Await.result(
          client.select(
            "SELECT * from %s WHERE str_field IS NULL AND int_field = 4567"
              .format(IntegrationSpec.pgTestTable)
          )(identity)
        )
      ...
      "return rows from UPDATE...RETURNING" in {
        ...
        val preparedQuery = client.prepareAndQuery(
          "UPDATE %s SET str_field = $1 where int_field = 4567 RETURNING *"
            .format(IntegrationSpec.pgTestTable),
          Param("hello_updated")
        )(identity)
      ...
      "return rows from DELETE...RETURNING" in {
        ...
        val preparedQuery = client.prepareAndQuery(
          "DELETE FROM %s where int_field = 9012 RETURNING *".format(
            IntegrationSpec.pgTestTable
          )
        )(identity)
      ...
      "execute an UPDATE...RETURNING that updates nothing" in {
        ...
        insertSampleData(client)
        val preparedQuery = client.prepareAndQuery(
          "UPDATE %s SET str_field = $1 where str_field = $2 RETURNING *"
            .format(IntegrationSpec.pgTestTable),
          Param("hello_updated"),
          Param("xxxx")
        )(identity)
      ...
      "execute a DELETE...RETURNING that deletes nothing" in {
        ...
        val preparedQuery = client.prepareAndQuery(
          "DELETE FROM %s WHERE str_field=$1".format(
            IntegrationSpec.pgTestTable
          ),
          Param("xxxx")
        )(identity)
      ...
      "create an extension using CREATE EXTENSION" in {
        if (user == "postgres") {
          val client = getClient
          val result =
            client.prepareAndExecute("CREATE EXTENSION IF NOT EXISTS hstore")
          Await.result(result)
        }
      }
      ...
      "throw a ServerError" when {
        "query has error" in {
          ...
          val selectQuery = client.select(
            "SELECT * FROM %s WHERE unknown_column='hello_updated'".format(
              IntegrationSpec.pgTestTable
            )
          )(identity)
      ...
      "generate unique names per prepared statement" in {
        ...
        val concurrentQueries = (1 to 1000).map {
          case number if number % 2 == 0 =>
            client.prepareAndExecute(
              "INSERT INTO %s (str_field, int_field, double_field, bool_field) VALUES ($1, $2, $3, $4)"
                .format(IntegrationSpec.pgTestTable),
              number.toString,
              number,
              number,
          ...
          case number =>
            client.prepareAndExecute(
              "INSERT INTO %s (str_field) VALUES ($1)".format(
                IntegrationSpec.pgTestTable
              ),
              number.toString
            )
