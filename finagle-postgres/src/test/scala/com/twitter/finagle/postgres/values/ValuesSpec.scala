...
class ValuesSpec extends Spec with ScalaCheckDrivenPropertyChecks {
  ...
  for {
    ...
    def test[T: Arbitrary](decoder: ValueDecoder[T], encoder: ValueEncoder[T])(
        send: String,
        typ: String,
      ...
      try {
        forAll { (t: T) =>
          ...
          val bytes = Await
            .result(
              client.prepareAndQuery(s"SELECT $send($$1::$typ) AS out", str)(
                _.get[Array[Byte]]("out")
              )
            )
            .head
          val textString = Await
            .result(
              client.prepareAndQuery(
                s"SELECT CAST($$1::$typ AS text) AS out",
                str
              )(_.get[String]("out"))
            )
            .head
          val binaryOut = decoder
            .decodeBinary(recv, Unpooled.wrappedBuffer(bytes), client.charset)
            .get
          ...
          val encodedBinary = encoder
            .encodeBinary(t, client.charset)
            .getOrElse(fail("Binary encoding produced null"))
          val encodedText =
            encoder.encodeText(t).getOrElse(fail("Text encoding produced null"))

          val binaryInOut = decoder
            .decodeBinary(recv, encodedBinary.duplicate(), client.charset)
            .get
          ...
          if (!nonDefault) {
            val List(rowBinary) = Await.result(
              ResultSet(
                Array(Field("column", 1, 0)),
                StandardCharsets.UTF_8,
                AsyncStream.fromSeq(List(DataRow(Array(Some(encodedBinary))))),
                Map(0 -> TypeSpecifier(recv, typ, 0)),
                ValueDecoder.decoders
              ).rows.toSeq.map(_.toList)
            )

            assert(
              tester(rowBinary.getAnyOption("column").get.asInstanceOf[T], t)
            )
            ...
            assert(
              tester(rowText.getAnyOption("column").get.asInstanceOf[T], t)
            )
    ...
    "ValueDecoders" should {
      "parse varchars" in test(ValueDecoder.string, ValueEncoder.string)(
        "varcharsend",
        "varchar"
      )
      "parse text" in test(ValueDecoder.string, ValueEncoder.string)(
        "textsend",
        "text"
      )
      "parse booleans" in test(ValueDecoder.boolean, ValueEncoder.boolean)(
        "boolsend",
        "boolean",
        b => if (b) "t" else "f"
      )
      "parse shorts" in test(ValueDecoder.int2, ValueEncoder.int2)(
        "int2send",
        "int2"
      )
      "parse ints" in test(ValueDecoder.int4, ValueEncoder.int4)(
        "int4send",
        "int4"
      )
      "parse longs" in test(ValueDecoder.int8, ValueEncoder.int8)(
        "int8send",
        "int8"
      )
      // precision seems to be an issue when postgres parses text floats
      "parse floats" in test(ValueDecoder.float4, ValueEncoder.float4)(
        "float4send",
        "numeric"
      )
      "parse doubles" in test(ValueDecoder.float8, ValueEncoder.float8)(
        "float8send",
        "numeric"
      )
      "parse numerics" in test(ValueDecoder.bigDecimal, ValueEncoder.numeric)(
        "numeric_send",
        "numeric"
      )

      tzSensitive { tz =>
        s"parse timestamps when local timezone is ${tz.getID}" in test(
          ValueDecoder.localDateTime,
          ValueEncoder.timestamp
        )(
          "timestamp_send",
          "timestamp",
          ts =>
            java.sql.Timestamp
              .from(ts.atZone(ZoneId.systemDefault()).toInstant)
              .toString,
          (a, b) =>
      ...
      "parse uuids" in test(ValueDecoder.uuid, ValueEncoder.uuid)(
        "uuid_send",
        "uuid"
      )
      "parse dates" in test(ValueDecoder.localDate, ValueEncoder.date)(
        "date_send",
        "date"
      )
      ...
      "parse jsonb" in {
        ...
        val createBuffer = () => {
          ValueEncoder[JSONB]
            .encodeBinary(JSONB(json.getBytes), Charset.defaultCharset())
            .get
        }
        ...
        val decoded = ValueDecoder[JSONB]
          .decodeBinary("", createBuffer(), Charset.defaultCharset())
          .get()
        JSONB.stringify(decoded) must equal(json)
      }

      "parse json" in test(ValueDecoder.string, ValueEncoder.string)(
        "json_send",
        "json"
      )(
        Arbitrary(
