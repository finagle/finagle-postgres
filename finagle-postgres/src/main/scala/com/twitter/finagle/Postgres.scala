...
object Postgres {
  ...
  object PostgresDefaultMonitor extends com.twitter.util.Monitor {
    ...
    def handle(exc: Throwable): Boolean = exc match {
      case ServerError(_, _, _, Some(sqlState), _, _, _)
  ...
  object Database {
    implicit object param extends RequiredParam[Database]("Database")
  }

  case class CustomTypes(
      types: Option[Map[Int, postgres.PostgresClient.TypeSpecifier]]
  ) extends AnyVal
  object CustomTypes { implicit val param = Param(CustomTypes(None)) }

  case class CustomReceiveFunctions(
      functions: PartialFunction[String, ValueDecoder[T] forSome { type T }]
  ) extends AnyVal
  object CustomReceiveFunctions {
    implicit val param = Param(CustomReceiveFunctions(PartialFunction.empty))
  }
  ...
  private def defaultStack = StackClient
    .newStack[PgRequest, PgResponse]
  ...
  private def channelInitializer(params: Stack.Params) = {
    val SslClientEngineFactory.Param(sslFactory) =
      params[SslClientEngineFactory.Param]
    val SslClientSessionVerifier.Param(sessionVerifier) =
      params[SslClientSessionVerifier.Param]
    ...
    (pipeline: ChannelPipeline) => {
      pipeline
        ...
        .addLast(
          "packet_to_backend_messages",
          new BackendMessageDecoder(new BackendMessageParser)
        )
        .addLast(
          "backend_messages_to_postgres_response",
          new PgClientChannelHandler(
            sslFactory,
            sessionVerifier,
            ssl,
            ssl.nonEmpty
  ...
  private def mkTransport(params: Stack.Params, addr: SocketAddress) =
    Netty4Transporter.raw[PgRequest, PgResponse](
      ...
      addr,
      params + Transport.ClientSsl(
        None
      ) // we want to give this param to Postgres but not directly to transport
  ...
  case class Client(
    ...
    def newRichClient(): postgres.PostgresClientImpl = {
      ...
      val CustomReceiveFunctions(customReceiveFunctions) =
        params[CustomReceiveFunctions]
      ...
      new postgres.PostgresClientImpl(
        client,
        id,
        customTypes,
    ...
    def newRichClient(addr: String): postgres.PostgresClientImpl =
      dest(addr).newRichClient()
    def newRichClient(addr: Name): postgres.PostgresClientImpl =
      dest(addr).newRichClient()
    ...
    def withBinaryParams(enable: Boolean = true) = configured(
      BinaryParams(enable)
    )
    def withBinaryResults(enable: Boolean = true) = configured(
      BinaryResults(enable)
    )
    def database(database: String) = configured(Database(database))
    def withCustomTypes(
        customTypes: Map[Int, postgres.PostgresClient.TypeSpecifier]
    ) =
      configured(CustomTypes(Some(customTypes)))
    def withDefaultTypes() = configured(
      CustomTypes(Some(postgres.PostgresClient.defaultTypes))
    )
    def withCustomReceiveFunctions(
        receiveFunctions: PartialFunction[String, ValueDecoder[
          T
        ] forSome { type T }]
    ) =
    ...
    def withRetryPolicy(policy: RetryPolicy[Try[Nothing]]) = configured(
      Retries.Policy(policy)
    )

    def conditionally(bool: Boolean, conf: Client => Client) =
      if (bool) conf(this) else this

    protected def newTransporter(
        addr: SocketAddress
    ): Transporter[In, Out, Context] = mkTransport(params, addr)

    protected def newDispatcher(transport: Transport[In, Out] {
      type Context <: Client.this.Context
    }): Service[PgRequest, PgResponse] = {
      new Dispatcher(
        transport,
    ...
    protected def copy1(
        ...
        params: Params
    ): Client { type In = Client.this.In; type Out = Client.this.Out } =
      copy(stack, params)
  }

  private class Dispatcher(
      transport: Transport[PgRequest, PgResponse],
      statsReceiver: StatsReceiver
  ) extends SerialClientDispatcher[PgRequest, PgResponse](
        transport,
        statsReceiver
      ) {
    ...
    override def apply(
      ...
      case PgRequest(Terminate, true) =>
        transport
          .write(req)
          .flatMap(_ => transport.close().map(_ => Terminated))
      case _ => super.apply(req)
    }
  }

  private object PrepConnection
      extends Stack.ModuleParams[ServiceFactory[PgRequest, PgResponse]] {
    def parameters: Seq[Param[_]] = Nil
    def role: Role = StackClient.Role.prepConn
    val description =
      "Prepare the PostgreSQL connection by authenticating and managing connection failures"
    def make(
        params: Params,
        next: ServiceFactory[PgRequest, PgResponse]
    ): ServiceFactory[PgRequest, PgResponse] = {
      val User(user) = params[User]
      val Password(password) = params[Password]
      val Database(db) = params[Database]
      val Transport.ClientSsl(ssl) = params[Transport.ClientSsl]
      new AuthenticationProxy(
        new HandleErrorsProxy(next),
        user,
        password,
        db,
        ssl.nonEmpty
  ...
  private val defaultResponseClassifier: service.ResponseClassifier = {
    ...
    case ReqRep(a, Throw(SqlState(sqlState))) if sqlState startsWith "XX" =>
      ResponseClass.NonRetryableFailure
    case ReqRep(a, Throw(SqlState(sqlState))) if sqlState startsWith "58" =>
      ResponseClass.NonRetryableFailure
    // Operator intervention - these should be failures as well
    case ReqRep(a, Throw(SqlState(sqlState))) if sqlState startsWith "57" =>
      ResponseClass.NonRetryableFailure
    case ReqRep(a, Throw(SqlState(_)))    => ResponseClass.Success
    case ReqRep(a, Throw(ClientError(_))) => ResponseClass.Success
  }

  private val defaultBackoff = Backoff.exponential(
