...
/** Typeclass supporting type-safe decoding of values from PostgreSQL in text
  * and binary formats
  */
trait ValueDecoder[T] {

  /** Attempt to decode the given text with the Postgres data type represented
    * by `recv` into [[T]]
    * @param recv
  ...
  /** Attempt to decode the given bytes with the Postgres data type represented
    * by `recv` into [[T]]
    * @param recv
    *   The name of PostgreSQL's receive function for the text's data type
    * @param bytes
    *   The bytes to be decoded; must be at the correct position (beginning of
  ...
  /** Create a new [[ValueDecoder]] which is the composition of this decoder
  ...
  def map[U](fn: T => U): ValueDecoder[U] = new ValueDecoder[U] {
    def decodeText(recv: String, text: String) =
      ValueDecoder.this.decodeText(recv, text).map(fn)
...
object ValueDecoder {
  ...
  def instance[T](
      text: String => Try[T],
      binary: (ByteBuf, Charset) => Try[T]
  ): ValueDecoder[T] =
    new ValueDecoder[T] {
      def decodeText(recv: String, s: String) = text(s)
      def decodeBinary(recv: String, b: ByteBuf, charset: Charset) =
        binary(b, charset)
    }

  def instance[T](
      text: (String, String) => Try[T],
      binary: (String, ByteBuf, Charset) => Try[T]
  ): ValueDecoder[T] =
    new ValueDecoder[T] {
      def decodeText(recv: String, s: String) = text(recv, s)
      def decodeBinary(recv: String, b: ByteBuf, charset: Charset) =
        binary(recv, b, charset)
    }
  ...
  implicit val boolean: ValueDecoder[Boolean] = instance(
    s => Return(s == "t" || s == "true"),
    (b, c) => Try(b.readByte() != 0)
  )
  ...
  implicit val int2: ValueDecoder[Short] =
    instance(s => Try(s.toShort), (b, c) => Try(b.readShort()))
  implicit val int4: ValueDecoder[Int] =
    instance(s => Try(s.toInt), (b, c) => Try(b.readInt()))
  implicit val int8: ValueDecoder[Long] =
    instance(s => Try(s.toLong), (b, c) => Try(b.readLong()))
  implicit val float4: ValueDecoder[Float] =
    instance(s => Try(s.toFloat), (b, c) => Try(b.readFloat()))
  implicit val float8: ValueDecoder[Double] =
    instance(s => Try(s.toDouble), (b, c) => Try(b.readDouble()))
  val Oid = instance(
    s => Try(s.toLong),
    (b, c) => Try(Integer.toUnsignedLong(b.readInt()))
  )
  implicit val inet: ValueDecoder[InetAddress] = instance(
    s => Try(InetAddress.getByName(s)),
    (b, c) => Try(readInetAddress(b))
  )
  implicit val localDate: ValueDecoder[LocalDate] = instance(
    s => Try(LocalDate.parse(s)),
    (b, c) =>
      Try(
        LocalDate.now().`with`(JulianFields.JULIAN_DAY, b.readInt() + 2451545)
      )
  )
  ...
  implicit val localDateTime: ValueDecoder[LocalDateTime] = instance(
    ...
    (b, c) =>
      Try(
        LocalDateTime.ofInstant(DateTimeUtils.readTimestamp(b), ZoneOffset.UTC)
      )
  )
  ...
  implicit val zonedDateTime: ValueDecoder[ZonedDateTime] = instance(
    ...
    (
      ...
      Try(
        DateTimeUtils.readTimestamp(b).atZone(ZoneOffset.UTC)
      ) // binary mode does not provide the zone
  )
  implicit val offsetDateTime: ValueDecoder[OffsetDateTime] =
    zonedDateTime.map(_.toOffsetDateTime)
  ...
  implicit val uuid: ValueDecoder[UUID] = instance(
    s => Try(UUID.fromString(s)),
    (b, c) => Try(new UUID(b.readLong(), b.readLong()))
  )
  ...
  implicit val javaBigDecimal: ValueDecoder[java.math.BigDecimal] =
    bigDecimal.map(_.bigDecimal)
  ...
  val never: ValueDecoder[Nothing] = instance(
    (recv, _) =>
      Throw(new NoSuchElementException(s"No decoder available for $recv")),
    (recv, _, _) =>
      Throw(new NoSuchElementException(s"No decoder available for $recv"))
  )

  def unknownBinary(t: (ByteBuf, Charset)): Try[Any] = Return(
    Buffers.readBytes(t._1)
  )
  ...
  val decoders: PartialFunction[String, ValueDecoder[T] forSome { type T }] = {
    ...
    case "charrecv" | "namerecv" | "varcharrecv" | "xml_recv" | "json_recv" |
        "textrecv" | "bpcharrecv" | "cstring_recv" | "citextrecv" |
        "enum_recv" =>
      string
    case "int8recv" => int8
    case "int4recv" => int4
    ...
    case "timestamptz_recv" =>
      instant // TODO: this should be zonedDateTime since in text mode, using instant will lose the timezone
