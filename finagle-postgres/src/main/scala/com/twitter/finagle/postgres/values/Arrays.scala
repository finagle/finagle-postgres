...
object Arrays {
  ...
  object ArrayStringParser extends RegexParsers {
    ...
    val values = (value ~ valueComma.*) ^^ { case first ~ rest =>
      first :: rest
    } | value.? ^^ (_.toList)
    ...
    val maybeArrayValues =
      (maybeArrayValue ~ maybeArrayValueComma.*) ^^ { case first ~ rest =>
        first ::: rest.flatten
      } |
        maybeArrayValue.* ^^ (_.flatten)
    val root = "{" ~ maybeArrayValues ~ "}" ^^ { case _ ~ vs ~ _ =>
      vs
    }
    ...
    def apply(str: String) = parseAll(root, str) match {
      ...
      case Failure(_, _) | Error(_, _) =>
        Throw(new Exception("Failed to parse array string"))
    }
  ...
  def decodeArrayText[T](str: String, elemDecoder: ValueDecoder[T]) = {
    ArrayStringParser(str).flatMap { strings =>
      strings
        .map(str => elemDecoder.decodeText("", str))
        .foldLeft[Try[Queue[T]]](Return(Queue.empty[T])) { (accum, next) =>
          accum.flatMap { current =>
            next.map(v => current enqueue v)
          }
        }
    }
  }
