...
private object DateTimeUtils {
  ...
  def format(i: Instant) =
    PostgresDateTimeFormat.withZone(ZoneOffset.UTC).format(i)
  ...
  private def query[T](f: TemporalAccessor => T): TemporalQuery[T] =
    new TemporalQuery[T] {
      override def queryFrom(temporal: TemporalAccessor): T = f(temporal)
    }

  def parseLocalDateTime(s: String) =
    PostgresDateTimeParser.parse(s, query(LocalDateTime.from))
  def parseInstant(s: String) = PostgresDateTimeParser
    .withZone(ZoneOffset.UTC)
    .parse(s, query(Instant.from))
  def parseZonedDateTime(s: String) =
    PostgresDateTimeParser.parse(s, query(ZonedDateTime.from))
  ...
  def readInterval(buf: ByteBuf) = {
    ...
    Interval(
      ...
      Period.ofMonths(months).plusDays(days)
    )
  }
  ...
  def writeInstant(instant: Instant) = {
    ...
    val micros =
      instant.getLong(ChronoField.MICRO_OF_SECOND) + seconds * 1000000
    val buf = Unpooled.buffer(8)
    buf.writeLong(micros - POSTGRES_EPOCH_MICROS)
    buf
  }

  def writeTimestamp(timestamp: LocalDateTime) = writeInstant(
    timestamp.atOffset(ZoneOffset.UTC).toInstant
  )

  def writeTimestampTz(timestamp: ZonedDateTime) = writeInstant(
    timestamp.toInstant
  )
  ...
  def writeInterval(interval: Interval) = {
    ...
    buf.writeLong(
      interval.timeDifference.getSeconds * 1000000 + interval.timeDifference.getNano / 1000
    )
    buf.writeInt(interval.dateDifference.getDays)
    buf.writeInt(
      interval.dateDifference.getYears * 12 + interval.dateDifference.getMonths
    )
    buf
  }
}
...
case class Interval(timeDifference: Duration, dateDifference: Period) {
  override def toString: String = {
    val timePart = Option(timeDifference).filter(_.getSeconds != 0).map { t =>
      ...
      val minutes =
        (totalMinutes - totalHours * 60).toString.reverse.padTo(2, '0').reverse
    ...
    val datePart = Option(dateDifference)
      .filterNot(dd => dd.getDays == 0 && dd.getMonths == 0 && dd.getYears == 0)
      .map { d =>
        def sign(i: Int) = if (i > 0) s"+$i" else i.toString
        val years =
          Option(d.getYears).filter(_ != 0).map(y => s"${sign(y)} years")
        val months =
          Option(d.getMonths).filter(_ != 0).map(m => s"${sign(m)} mons")
  ...
  override def equals(obj: scala.Any): Boolean = obj match {
    case Interval(td, dd) =>
      val e =
        td.getSeconds == timeDifference.getSeconds && td.getNano == timeDifference.getNano &&
...
object Interval {
  val regex =
    """(?:([\-+]?\d+) years? )?(?:([\-+]?\d+) mons? )?(?:([\-+]?\d+) days? )?(?:(-|\+)?(\d{2}):(\d{2}):(\d{2})(\.\d+)?)?""".r
  def parse(s: String) = {
    val t = regex.findFirstMatchIn(s).map { m =>
      val period = Period
        .ofYears(Option(m.group(1)).map(_.toInt).getOrElse(0))
      ...
      val duration = if (m.group(4) == "-") {
        Duration
          .ofHours(Option(m.group(5)).map(h => -1 * h.toLong).getOrElse(0))
        ...
        Duration
          .ofHours(Option(m.group(5)).map(_.toLong).getOrElse(0))
