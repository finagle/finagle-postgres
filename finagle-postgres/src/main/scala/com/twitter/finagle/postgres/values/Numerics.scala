...
private object Numerics {
  ...
  def readNumeric(buf: ByteBuf) = {
    ...
    if (bdDigits.length > 0) {
      val unscaled = bdDigits.foldLeft(BigDecimal(0)) { case (acc, n) =>
        acc * 10000 + n
      }

      val scaleFactor = (weight + 1 - len) * NumericDigitBaseExponent
      val unsigned =
        unscaled.bigDecimal.movePointRight(scaleFactor).setScale(displayScale)
  ...
  def writeNumeric(in: BigDecimal) = {
    ...
    def findDigits(i: BigInteger, current: List[Short] = Nil): List[Short] =
      if (i.signum() != 0) {
    ...
    val weight = {
      val powers10 = base10exponent(in.bigDecimal) - base10exponent(
        new java.math.BigDecimal(paddedUnscaled)
      )
