...
import com.twitter.finagle.postgres.connection.{
...
class HandleErrorsProxy(delegate: ServiceFactory[PgRequest, PgResponse])
  ...
  ): Future[Service[PgRequest, PgResponse]] = {
    for {
...
class AuthenticationProxy(
  ...
  private[this] def sendSslRequest(
      service: Service[PgRequest, PgResponse]
  ): Future[Option[PgResponse]] = {
    if (useSsl) {
      service(PgRequest(new SslRequestMessage)).map { response =>
        Some(response)
      }
    } else {
      Future.value(None)
  ...
  private[this] def handleSslResponse(
      optionalSslResponse: Option[PgResponse]
  ): Future[Unit] = {
      ...
      case PasswordRequired(encoding) =>
        password match {
          case Some(pass) =>
            val msg = encoding match {
              ...
              case Md5(salt) =>
                PasswordMessage(
                  new String(
                    Md5Encryptor.encrypt(user.getBytes, pass.getBytes, salt)
                  )
                )
            }
          ...
          case None =>
            Future.exception(
              Errors.client(
                "Password has to be specified for authenticated connection"
              )
            )
        }
 ...
 */
class BackendMessageDecoder(val parser: BackendMessageParser)
 ...
 */
class PacketDecoder(@volatile var inSslNegotation: Boolean)
  ...
  def decode(
    ...
    } else if (buffer.readableBytes() < 5) {} else {
...
class PgClientChannelHandler(
  ...
  override def channelRead(
    ...
    message match {
      case SwitchToSsl =>
        logger.ifDebug(
          "Got switchToSSL message; adding ssl handler into pipeline"
        )
        ...
        ctx.channel().remoteAddress() match {
          case i: InetSocketAddress =>
            ...
            val config = sslConfig.getOrElse(
              SslClientConfiguration(hostname = Some(i.getHostString))
            )
            val verifier = (s: SSLSession) =>
              sessionVerifier(address, config, s)
            ...
            sslHandler
              ...
              .addListener(new FutureListener[Channel] {
                override def operationComplete(
                    f: io.netty.util.concurrent.Future[Channel]
                ): Unit = {
                  if (
                    !Try(verifier(engine.getSession))
                      .onFailure { err =>
                        logger.error(
                          err,
                          "Exception thrown during SSL session verification"
                        )
                      }
                      .getOrElse(false)
                  ) {
          ...
          case _ =>
            ctx.fireExceptionCaught(
              new Exception("Unsupported socket address for SSL")
            )
        }
      case msg: BackendMessage =>
        try {
          ...
          case err @ WrongStateForEvent(evt, state) =>
            logger.error(
              s"Could not handle event $evt while in state $state; connection will be terminated",
              err
            )
  ...
  override def write(
    ...
    val (buf, out) = message match {
      ...
      case other =>
        logger.warning(
          s"Cannot convert message of type ${other.getClass.getName}... Skipping"
        )
        (Some(message), None)
    }

    buf.filter(_ => ctx.channel.isOpen) foreach { bytes =>
