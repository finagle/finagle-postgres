...
case class AggregatingAuthData(
...
case class StreamRows(
    complete: Promise[Unit],
    extended: Boolean,
    nextRow: Promise[AsyncStream[DataRow]] = new Promise
) extends ExtendedQueryState {
  val asyncStream: AsyncStream[DataRow] =
    AsyncStream.fromFuture(nextRow).flatten
}

case class AwaitRowDescription(types: Array[Int]) extends ExtendedQueryState

case class EmitOnReadyForQuery[R <: PgResponse](
    emit: StateMachine.TransitionResult[R]
) extends ExtendedQueryState
