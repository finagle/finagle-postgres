 ...
 */
class ConnectionStateMachine(state: State = AuthenticationRequired, val id: Int)
    extends StateMachine[Message, PgResponse, State] {
  private[this] val logger = Logger(
    s"${getClass.getName}.psql state machine:$id"
  )
  ...
  transition {
    ...
    case (SwitchToSsl, AwaitingSslResponse) =>
      (Some(SslSupportedResponse), AuthenticationRequired)
    case (SslNotSupported, AwaitingSslResponse) =>
      (Some(SslNotSupportedResponse), AuthenticationRequired)
  }

  transition {
    case (StartupMessage(_, _), AuthenticationRequired) =>
      (None, AuthenticationInProgress)
    case (AuthenticationOk(), AuthenticationInProgress) =>
      (None, AggregatingAuthData(Map(), -1, -1))
    ...
    case (PasswordMessage(_), AwaitingPassword) =>
      (None, AuthenticationInProgress)

    case (
          ErrorResponse(details),
          AuthenticationRequired | AuthenticationInProgress | AwaitingPassword
        ) =>
  ...
  transition {
    case (
          ...
          AggregatingAuthData(statuses, processId, secretKey)
        ) =>
      (
        None,
        AggregatingAuthData(
          statuses = statuses + (name -> value),
          processId,
          secretKey
        )
      )
    case (
          ...
          AggregatingAuthData(statuses, _, _)
        ) =>
    ...
    case (ErrorResponse(details), AggregatingAuthData(_, _, _)) =>
      (Some(Error(details)), AuthenticationRequired)
  }
  ...
  transition {
    ...
    case (ErrorResponse(details), Connected) =>
      (Some(Error(details)), Connected)
  }

  transition {
    case (ParameterDescription(types), AwaitParamsDescription) =>
      (None, AwaitRowDescription(types))
    case (RowDescription(fields), AwaitParamsDescription) =>
      (
        Some(
          RowDescriptions(
            fields.map(f => Field(f.name, f.fieldFormat, f.dataType))
          )
        ),
        Connected
      )
    case (NoData, AwaitParamsDescription) =>
      (Some(RowDescriptions(Array.empty)), Connected)
    case (ErrorResponse(details), AwaitParamsDescription) =>
      (Some(Error(details)), Connected)
  }
  ...
  transition {
    ...
    case (NoData, AwaitRowDescription(types)) =>
      (Some(RowDescriptions(Array.empty)), Connected)
    case (ErrorResponse(details), AwaitRowDescription(_)) =>
      (Some(Error(details)), Connected)
  }
  ...
  transition {
    ...
    case (ErrorResponse(details), Syncing) =>
      throw new IllegalStateException("Received an error in response to Sync")
  }
  ...
  transition {
    ...
    case (
          CommandComplete(
            CreateExtension | CreateFunction | CreateIndex | CreateTable |
            CreateTrigger | CreateType
          ),
          SimpleQuery
        ) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(1)))
    case (CommandComplete(DropTable), SimpleQuery) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(1)))
    case (CommandComplete(Insert(count)), SimpleQuery) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(count)))
    case (CommandComplete(Update(count)), SimpleQuery) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(count)))
    case (CommandComplete(Delete(count)), SimpleQuery) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(count)))
    case (CommandComplete(DiscardAll), SimpleQuery) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(1)))
    case (
          CommandComplete(Begin | Savepoint | Release | RollBack | Commit),
          SimpleQuery
        ) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(1)))
    case (CommandComplete(Set), SimpleQuery) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(1)))
    case (CommandComplete(Do), SimpleQuery) =>
      (None, EmitOnReadyForQuery(CommandCompleteResponse(1)))
  ...
  transition {
    ...
    case (CommandComplete(_), EmitOnReadyForQuery(old)) =>
      (None, EmitOnReadyForQuery(old))
  }

  transition {
    case (EmptyQueryResponse, ExecutePreparedStatement) =>
      (Some(SelectResult.Empty), Connected)
    case (
          CommandComplete(
            CreateExtension | CreateFunction | CreateIndex | CreateTable |
            CreateType | CreateTrigger
          ),
          ExecutePreparedStatement
        ) =>
      (Some(CommandCompleteResponse(1)), Connected)
    case (CommandComplete(DropTable), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(1)), Connected)
    case (CommandComplete(Insert(count)), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(count)), Connected)
    case (CommandComplete(Update(count)), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(count)), Connected)
    case (CommandComplete(Delete(count)), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(count)), Connected)
    case (CommandComplete(Begin), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(1)), Connected)
    case (CommandComplete(Set), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(1)), Connected)
    case (CommandComplete(Savepoint), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(1)), Connected)
    case (CommandComplete(RollBack), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(1)), Connected)
    case (CommandComplete(Commit), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(1)), Connected)
    case (CommandComplete(Do), ExecutePreparedStatement) =>
      (Some(CommandCompleteResponse(1)), Connected)
  ...
  fullTransition {
    case (ReadyForQuery(_), EmitOnReadyForQuery(response)) =>
      (Some(response), Connected)
    case (
          ErrorResponse(details),
          EmitOnReadyForQuery(StateMachine.Response(_))
        ) =>
      (Some(StateMachine.Response(Error(details))), Connected)
    case (
          ErrorResponse(details),
          EmitOnReadyForQuery(StateMachine.Complete(promise, _))
        ) =>
      val exn = Errors.server(Error(details), None)
  ...
  transition {
    case (Terminate, _) =>
      (Some(com.twitter.finagle.postgres.messages.Terminated), Terminated)
    case (_, Terminated) =>
      (Some(com.twitter.finagle.postgres.messages.Terminated), Terminated)
  }
