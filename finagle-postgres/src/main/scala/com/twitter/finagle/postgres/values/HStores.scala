...
object HStores {
  object HStoreStringParser extends RegexParsers {
    ...
    def items: Parser[Map[String, Option[String]]] = repsep(item, ", ") ^^ {
      l => l.toMap
    }

    def apply(input: String): Option[Map[String, Option[String]]] =
      parseAll(items, input) match {
  ...
  def formatHStoreString(hstore: Map[String, Option[String]]) = hstore
    .map { case (k, v) =>
      val key = s""""${k.replace("\"", "\\\"")}""""
      val value =
        v.map(str => s""""${str.replace("\"", "\\\"")}"""").getOrElse("NULL")
      s"""$key => $value"""
    }
    .mkString(",")
  ...
  def encodeHStoreBinary(
      hstore: Map[String, Option[String]],
      charset: Charset
  ) = {
    val buf = Unpooled.buffer()
    buf.writeInt(hstore.size)
    hstore foreach { case (key, value) =>
