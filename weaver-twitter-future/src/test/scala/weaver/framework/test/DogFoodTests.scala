...
object DogFoodTests extends TwitterFutureSuite {

  type Res = DogFood[Future]
  def sharedResource: Resource[Future, DogFood[Future]] =
    DogFood.make(new TwitterFuture())

  test("test suite reports successes events") { dogfood =>
    import dogfood.*
    runSuite(Meta.MutableSuiteTest).map { case (_, events) =>
      forEach(events)(isSuccess)
    }
  }
  ...
  test(
    ...
    _.runSuite(Meta.FailingTestStatusReporting).map { case (logs, _) =>
  ...
  test("test suite outputs logs for failed tests") {
    _.runSuite(Meta.FailingSuiteWithlogs).map { case (logs, _) =>
      val expected =
        s"""
             |- failure 0ms
      ...
      exists(extractLogEventAfterFailures(logs) { case LoggedEvent.Error(msg) =>
        msg
      }) { actual =>
  ...
  test("test suite outputs stack traces of exception causes") {
    _.runSuite(Meta.ErroringWithCauses).map { case (logs, _) =>
  ...
  test("failures with multi-line test name are rendered correctly") {
    _.runSuite(Meta.Rendering).map { case (logs, _) =>
      val actual = extractLogEventAfterFailures(logs) {
        case LoggedEvent.Error(msg) => msg
      }.get
  ...
  test("cancelled tests with multi-line test name are rendered correctly") {
    _.runSuite(Meta.Rendering).map { case (logs, _) =>
      val actual =
        extractLogEventBeforeFailures(logs) {
  ...
  test("expect.same delegates to show when an insteance is found") {
    _.runSuite(Meta.Rendering).map { case (logs, _) =>
      val actual =
  ...
  private def extractLogEventBeforeFailures(
  ...
  private def extractLogEventAfterFailures(
      logs: Chain[LoggedEvent]
  )(pf: PartialFunction[LoggedEvent, String]): Option[String] = {
    logs
      .dropWhile {
